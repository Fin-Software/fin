...
start() { _name=$1; . ../scripts/vendor.sh; pkg=/tmp/fin/$_name; $rm $pkg; $install $pkg; }  # . borrows $@
get() {
    ...
    _real="$(szip h -scrcSHA3-256 $srcs/$base | awk '/^SHA3-256 for/ {print $4}')"
    [ "$_real" = $_hash ] || { printf 'get %s !=\n%s\n' $_hash "$_real"; return 1; }
}
finish() {
    $find $pkg -type d -empty -delete; for _dir in $pkg/*; do $rm ${_dir##*/}; done; mv $pkg/* .; rmdir $pkg
}
...
llvm() {
    ...
    url=github.com/llvm/llvm-project/releases/download/llvmorg-$semv/$base
    ...
    (
        get e25036d460357ec4c542d5507feceb8a21d23f98f86234291a902392a8bcde07; set -f
            szip e -so $srcs/$base | szip x -o$pkg -si -ttar $(printf -- '-x!*/%s ' */bindings */docs */www */examples \
            */test */unittests llvm/benchmarks polly/lib/External/isl/test_inputs) $(printf --  \
            '-xr!%s ' .* Maintainers.* CREDITS.* *.png *.bmp) $(printf -- '*/%s ' $deps) >/dev/null
            set +f
        ...
        $find . -name CMakeLists.txt -exec sed -i '' -E "$_guard" {} +
    ); finish
}
other() {
    start other; semv=master base=LRSTAR-$semv.tar.gz url=github.com/p7r0x7/LRSTAR/archive/${base#*-}
    (
        get f6b33c1ea42ebdd1793728daa82a30095848345958b9498b3fbb6e9da00534a2; set -f
            szip e -so $srcs/$base | szip x -o$pkg -si -ttar $(printf -- '-i!*/*/*.%s ' h hpp cpp txt grm lgr) \
            -x!*/bin -xr!?G.*.txt >/dev/null
            set +f
    ...
    (
        get faf88aa26fa06f469e05fcb48434804905f70eb36c7760916f2f4de4287c85f6; set -f
            szip e -so $srcs/$base | szip x -o$pkg -si -ttar $(printf -- '-x!*/%s ' zlibWrapper lib/deprecated \
            lib/legacy build/meson build/VS* *T*.md Package.swift CHANGELOG)            \
            $(printf -- '-xr!%s ' contrib/ example*/ programs/ tests/ *.png .*) >/dev/null
            set +f
    ) &
    ...
    (
        get d60ffcfad05908d1efb932177340d2c80c9db123cf9eadff5657945f214a2214; set -f
            szip e -so $srcs/$base | szip x -o$pkg -si -ttar $(printf -- '-i!*%s ' c h in) -i!CMakeLists.txt >/dev/null
            set +f
    ) & wait; finish
}
