...
settings() {
	set -eu; umask 0022; tabs -4; install='install -dm 0755'
...
libs() {
	_flags='-pipe -flto=thin -fno-omit-frame-pointer -mllvm -polly -mllvm -polly-vectorizer=stripmine'
	...
	[ -f .build/hash ] || {
		...
		clang -fuse-ld="$(command -v ld64.lld)" -O3 $_flags -Wl,$gc /tmp/fin/*.ll -o .build/hash		
	}
	[ "$(.build/hash vendor)" = '3j8Qjuv+lRuTkMC/0HC7bEz8MYXlzVcj7AArZ8HGbF8=' ]
	
	case ${target%-*} in
		darwin) set -- -DCMAKE_LINKER="$(command -v ld64.lld)" -DCMAKE_OSX_SYSROOT="$(xcrun --show-sdk-path)" ;;
		android|linux) set -- -DCMAKE_LINKER="$(command -v ld.lld)" ;;
		windows) set -- -DCMAKE_LINKER="$(command -v lld-link)" ;;
	esac
	set -- -Wno-dev -G Ninja "$@" \
		...
		-DCMAKE_CXX_FLAGS_RELEASE="-O3 $_flags" \
		-DCMAKE_C_FLAGS_RELEASE="-O3 $_flags" \
		-DCMAKE_EXE_LINKER_FLAGS_RELEASE=-O3 \
		-DCMAKE_OBJCOPY=false \
		-DCMAKE_RANLIB=false \
		-DCMAKE_NM=false

	buildzstd=.build/$target/buildzstd prefixzstd=.build/$target/installzstd
	...
	buildllvm=.build/$target/buildllvm prefixllvm=.build/$target/installllvm
